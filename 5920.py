...
url_count = 4  # 每个账号生成几条链接
each_url_resend = 2  # 每个链接复用几次
...
deadline_minute = 59
deadline_second = 59
deadline_microsecond_avg = 950000  # 根据cpu network 调整此参数
test_data = {}
host = "https://"
already_push = set()
key = 'convertUrlNew'
...
push_url = "http://www.pushplus.plus/send?token={}&title={}&content={}"
...
def main():
    ...
    for account in cks:
        ...
        if receiveKey == -1:
            return
        s = 0
        print(f'正在生成账号{account["name"]}共计{url_count * each_url_resend}条抢券链接')
